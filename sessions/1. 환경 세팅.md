# 1. 계정 생성

## ⏰ 소요 시간

**25분**

***

## 📖 계정 생성

GitHub Copilot과 함께 업무 자동화를 시작하기 위해 계정을 생성합니다.

### Step 1: GitHub 계정 만들기

* 노트북 (macOS 또는 Windows)
* GitHub 계정 ([github.com](https://github.com) 에서 무료 생성 가능)
* Copilot Pro 30일 무료 신청 ([https://github.com/github-copilot/pro](https://github.com/github-copilot/pro))
* 집중할 수 있는 환경 ☕

***

## 💻 옵션 1: IntelliJ 에서 진행하기

### Step 1: IntelliJ IDEA 설치

1. [IntelliJ IDEA 다운로드](https://www.jetbrains.com/idea/download/)
2. **Community Edition (무료)** 또는 **Ultimate Edition** 선택
3. 다운로드한 설치 파일 실행
4. 설치 마법사 따라 진행 (기본 설정 권장)
5. 설치 완료 후 IntelliJ IDEA 실행

### Step 2: GitHub Copilot 플러그인 설치

1. **IntelliJ IDEA 열기**
2.  **GitHub Copilot 검색 및 설치** 1.

    ```
    <figure><img src=".gitbook/assets/image (3).png" alt=""><figcaption></figcaption></figure>
    ```
3.  IDE 다시 시작 1.

    ```
    <figure><img src=".gitbook/assets/image (5).png" alt=""><figcaption></figcaption></figure>
    ```

### Step 3: 새 프로젝트 생성

1.  실습을 진행할 프로젝트를 생성합니다. 1.

    ```
    <figure><img src=".gitbook/assets/image.png" alt=""><figcaption></figcaption></figure>
    ```
2.  왼쪽 메뉴에서 '빈 프로젝트' 를 선택하고 '이름'과 '위치'를 지정하고 생성 버튼을 누릅니다. 1.

    ```
    <figure><img src=".gitbook/assets/image (1).png" alt=""><figcaption></figcaption></figure>
    ```

### Step 4: 로그인 및 권한 승인

1.  하단 오른쪽에 있는 아이콘을 누른 뒤 " Login to GitHub" 버튼을 클릭합니다. 1.

    ```
    <figure><img src=".gitbook/assets/unknown (1).png" alt=""><figcaption></figcaption></figure>
    ```
2.  Device Code 를 복사하고 \[Copy and Open] 버튼을 클릭합니다. 1.

    ```
    <figure><img src=".gitbook/assets/image (8).png" alt=""><figcaption></figcaption></figure>
    ```
3.  로그인한 계정을 선택해서 \[Continue] 버튼을 누릅니다.

    <figure><img src=".gitbook/assets/image (2).png" alt=""><figcaption></figcaption></figure>
4.  Device Activation 코드를 붙여넣고 Continue 를 누릅니다. 1.

    ```
    <figure><img src=".gitbook/assets/image (9).png" alt=""><figcaption></figcaption></figure>
    ```
5. 이어서 나온 화면에서 Authorize 를 눌러서 로그인을 완료합니다.

### Step 5: Copilot 활성화 확인하기

1.  ntelliJ 오른쪽 하단 상태 바에서 Copilot 아이콘을 확인합니다. 1.

    ```
    <div align="left"><figure><img src=".gitbook/assets/image (12).png" alt=""><figcaption></figcaption></figure></div>
    ```

### Step 6: Copilot Chat 열기

1.  활성화된 버튼을 누르고 \[Open Chat] 버튼을 누릅니다. 1.

    ```
    <figure><img src=".gitbook/assets/image (13).png" alt=""><figcaption></figcaption></figure>
    ```
2.  "안녕! 코파일럿!' 을 입력하고 응답을 확인합니다. 1.

    ```
    <figure><img src=".gitbook/assets/image (7).png" alt=""><figcaption></figcaption></figure>
    ```

***

## 💻 옵션 2: VS Code 에서 진행하기

### Step 1: macOS 사용자 설치

1. [Visual Studio Code 다운로드](https://code.visualstudio.com/Download)
2. 다운로드한 `.zip` 파일 압축 해제
3. `Visual Studio Code.app`을 `Applications` 폴더로 이동
4. Spotlight 검색 (⌘ + Space)에서 "Visual Studio Code" 입력하여 실행

### Step 2: Windows 사용자 설치

1. [Visual Studio Code 다운로드](https://code.visualstudio.com/Download)
2. 다운로드한 `.exe` 파일 실행
3. 설치 마법사 따라 진행 (기본 설정 권장)
4. 설치 완료 후 VS Code 실행

### Step 3: GitHub Copilot 확장 설치하기

1. **VS Code 열기**
2. **확장(Extensions) 패널 열기**
3. **GitHub Copilot 검색 및 설치**

```
검색창에 "GitHub Copilot" 입력
→ "GitHub Copilot" (by GitHub) 찾기
→ "Install" 버튼 클릭
```

### Step 4: GitHub Copilot Chat 설치

```
검색창에 "GitHub Copilot Chat" 입력
→ "GitHub Copilot Chat" (by GitHub) 찾기
→ "Install" 버튼 클릭
```

### Step 5: 로그인 및 권한 승인

* 설치 후 "Sign in to use GitHub Copilot" 알림 표시
* "Sign In to GitHub" 버튼 클릭
* 브라우저에서 GitHub 계정 로그인
* 권한 승인 (Authorize)

### Step 6: Copilot 활성화 확인하기

* VS Code 오른쪽 하단에 Copilot 아이콘 확인
* 초록색이면 정상, 빨간색이면 오류

### Step 7: Copilot Chat 열기

1. 왼쪽 사이드바에서 Chat 아이콘 클릭
2. "Hello, Copilot!" 입력하여 응답 확인

***

## 📚 다음 세션 미리보기

**2. VS Code 에서 진행하기** 또는 **3. IntelliJ 에서 진행하기**에서는:

* 개발 환경(IDE) 설치
* GitHub Copilot 확장/플러그인 설치
* Copilot 로그인 및 활성화 확인

**소요 시간:** 10분\
**준비물:** 없음 (원하는 IDE를 선택하세요!)
