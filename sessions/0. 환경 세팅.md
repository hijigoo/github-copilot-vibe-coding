...
### Step 1: Copilot에게 가상환경 생성 및 활성화 요청하기

Copilot이 직접 가상환경을 생성하고 활성화하도록 해봅시다!
...
현재 프로젝트 폴더에 Python 가상환경 .venv를 생성하고 활성화해줘.
터미널 명령어를 실행 가능한 코드 블록으로 제공해줘.
```

3. **Copilot이 제공한 명령어를 바로 실행**

Copilot이 다음과 같은 명령어를 제공할 것입니다. **코드 블록의 실행 버튼을 클릭**하거나 복사해서 터미널에 붙여넣으세요:
...
💡 **Tip**: Copilot이 생성한 코드 블록 오른쪽의 ▶️ 버튼을 클릭하면 터미널에서 바로 실행됩니다!
